...
model = joblib.load('E:/Milk/Deploy/new/RFC.pkl')
...
def predict_view(request):
    if request.method == 'POST':
        print('IF')
        fieldss = ['pH', 'Temperature', 'Taste', 'Odour', 'Fat', 'Turbidity', 'Colour']
        ...
        output = prediction[0]
        if output == 'high':
            output = 'High Grade Milk'
        elif output == 'low':
            output = 'Low Grade Milk'
        elif output == 'medium':
            output = 'Medium Grade Milk'
        print(features)
        print(output)
        if form.is_valid():
            print('saving')
